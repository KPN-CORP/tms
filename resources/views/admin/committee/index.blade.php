<x-app-layout>
    ...
    <div class="p-6 space-y-6 max-w-5xl">
                                ...
                                <tr class="align-top hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('admin.committee.index', array_filter(['approval_type' => $set['approval_type'], 'business_unit_id' => $set['business_unit_id'], 'department_id' => $set['department_id']])) }}#editor"
        ...
        <form method="GET" action="{{ route('admin.committee.index') }}" class="bg-white rounded-xl shadow p-6 grid {{ $type === 'idea' ? 'grid-cols-3' : 'grid-cols-2' }} gap-4">
            <div>
        ...
        <form method="POST" action="{{ route('admin.committee.store') }}" class="bg-white rounded-xl shadow p-6 space-y-4">
            ...
            @endif
            <p class="text-sm text-gray-500">Approval: <span class="font-semibold">{{ $types[$type] }}</span> — BU: <span class="font-semibold">{{ optional($businessUnits->firstWhere('id', $selectedBuId))->name }}</span>@if($type === 'idea') — Unit: <span class="font-semibold">{{ $selectedDeptId ? optional($departments->firstWhere('id', $selectedDeptId))->name : 'Semua Department (BU-wide)' }}</span>@endif</p>
            ...
            @for($layer = 1; $layer <= $maxLayers; $layer++)
                <div class="flex items-center gap-4">
                    ...
                    <select name="layers[{{ $layer }}]"
                            class="flex-1 border rounded-lg px-4 py-2 focus:ring focus:ring-red-200">
                        <option value="">— none —</option>
                        @foreach($users as $u)
                            <option value="{{ $u->id }}" @selected(($assignments[$layer] ?? null) == $u->id)>
                                {{ $u->name }} ({{ optional($u->businessUnit)->name ?? '-' }})
                            </option>
                        @endforeach
                    </select>
                </div>
